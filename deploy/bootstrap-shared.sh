...
4. Check container status:
   docker compose ps
5. In multi-user mode, let users connect to /mcp and call auth_configure_session once.
   In single-account mode, set HUAWEICLOUD_AK/SK/REGION and MCP_AUTH_STATIC_TOKEN in .env,
   then configure the client with Authorization: Bearer <MCP_AUTH_STATIC_TOKEN>.
