...
describe("createServer tool registration", () => {
  it("registers only the 276 product tools in stdio mode", () => {
  ...
  it("registers 278 tools including auth tools in http mode", () => {
  ...
  it("reuses a cached tools/list result across repeated requests", async () => {
    ...
    expect((firstResult as { tools?: unknown[] }).tools).toHaveLength(278);
  });
