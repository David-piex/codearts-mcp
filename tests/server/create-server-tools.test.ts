...
describe("createServer tool registration", () => {
  it("registers only the 262 product tools in stdio mode", () => {
  ...
  it("registers 264 tools including auth tools in http mode", () => {
    const server = createHttpServer();

    const toolNames = readRegisteredToolNames(server);

    expect(toolNames).toHaveLength(264);
  ...
  it("reuses a cached tools/list result across repeated requests", async () => {
    ...
    expect((firstResult as { tools?: unknown[] }).tools).toHaveLength(264);
  });
