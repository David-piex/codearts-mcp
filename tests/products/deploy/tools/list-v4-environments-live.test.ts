...
if (hasLiveEnv(process.env)) {
  describe("createDeployListV4EnvironmentsHandler live", () => {
    ...
    const handler = createDeployListV4EnvironmentsHandler(client);
    const projectId = readProjectId(process.env);

    it("reaches the published v4 environments route and currently returns an empty list on this tenant", async () => {
  ...
  describe.skip("createDeployListV4EnvironmentsHandler live", () => {});
}
