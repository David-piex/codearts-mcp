...
const toolStatusSummaryMeta: Record<ModuleName, ToolStatusSummaryMeta> = {
  Req: {
    summary:
      "Expanded Req surface with current-user info/role, project bug/summary/statistics reads, tree count/list, work-item tag/index-count reads, project work-item history, child work items, work-hours, issue image upload/download, attachment upload/download/delete, associated wiki reads, plan work-item management, project due-days-after/workhour-config reads, and status/public-config/cache support",
    conclusion:
      "The Req MCP surface has grown from the original 8-tool core to current-user info/role reads, project/module/member/iteration/plan/work-item/work-item-tree/project-wide-record-history/child-work-item collaboration, work-item tag/index-count reads, project bug/demand-statistic/project summary/work-item completion-rate reads, work-hours, issue image upload/download, attachment upload/download/delete, associated issue/test-case/wiki reads, plan work-item management, plan image update, plan-context work item creation, project due-days-after/workhour-config reads, status/status-attribute/status-detail/workflow-config/template/template-config/custom-field/status-rule-flag/status-config/optional-status-config/tracker-handler and project-public-config reads, field/cache reads, and initial board work-item read coverage; core live loops are validated while deeper member, batch, plan, plan work-item management, plan image update, plan-context work item creation, summary/statistics read samples, cache-read, and board samples remain pending."
  },
  Repo: {
...
const readmeModuleNumbersMeta: Record<ModuleName, ReadmeModuleNumbersMeta> = {
  Req: {
    liveStatus: "Partial",
    breakdown:
      "Expanded Req surface with current-user info/role reads, project bug/summary/statistics reads, work-item-tree count/list, work-item tag/index-count reads, project work-item history reads, child work-item reads, work-hours, issue image upload/download, attachment upload/download/delete, associated wiki reads, plan work-item management, plan image update, plan-context work item creation, project due-days-after/workhour-config reads, status/status-attribute/status-detail/workflow-config/template/template-config/custom-field/status-rule-flag/status-config/optional-status-config/tracker-handler and project-public-config reads plus field/cache reads and board work-item reads; see `docs/wiki/Req-Live-Validated.md` for validated paths and remaining live-depth gaps"
  },
  Repo: {
