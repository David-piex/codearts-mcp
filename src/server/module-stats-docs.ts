...
const readWriteMatrixMeta: Record<ModuleName, ReadWriteMatrixMeta> = {
  Req: {
    live: "Partial",
    keyGaps:
      "Scrum project, module, member, iteration, plan, work-item, comment, association, status/status-attribute/status-detail/workflow-config/template/template-config/custom-field/status-rule-flag/status-config/optional-status-config/tracker-handler/project-public-config reads, board work-item reads, and field/cache reads are implemented; deeper live coverage is still expanding for member, batch, plan, new status/public-config read paths, cache reads, and board samples"
  },
  Repo: {
    live: "Validated",
    keyGaps:
