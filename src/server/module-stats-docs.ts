...
const implementationStatusMeta: Record<ModuleName, ImplementationStatusMeta> = {
  Req: {
    live: "Partial",
    notes:
      "Req now covers Scrum project, module, member, iteration, work-item, comment, association, related-user, status, workflow-config, template, template-config, custom-field, status-rule-flag, tracker-handler, board work-item reads, field/cache reads, and flow-transition tools. Core project/work-item paths have live coverage; member, batch-operation, new config-read paths, cache reads, and board reads still need deeper live coverage."
  },
  Repo: {
    live: "Validated",
    notes:
...
const toolStatusSummaryMeta: Record<ModuleName, ToolStatusSummaryMeta> = {
  Req: {
    summary: "Expanded Scrum surface with configuration and cache read support",
    conclusion:
      "The Req MCP surface has grown from the original 8-tool core to project/module/member/iteration/work-item collaboration plus status/workflow/template/template-config/custom-field/status-rule-flag/tracker-handler configuration reads, field/cache reads, and initial board work-item read coverage; core live loops are validated while deeper member, batch, config-read, cache-read, and board samples remain pending."
  },
  Repo: {
...
const readmeModuleNumbersMeta: Record<ModuleName, ReadmeModuleNumbersMeta> = {
  Req: {
    liveStatus: "Partial",
    breakdown:
      "Expanded Scrum surface with status/workflow/template/template-config/custom-field/status-rule-flag/tracker-handler config reads plus field/cache reads and board work-item reads; see `docs/wiki/Req-Live-Validated.md` for validated paths and remaining live-depth gaps"
  },
  Repo: {
