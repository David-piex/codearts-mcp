...
export function mapPipelinePluginInputs(
  input: Omit<PipelinePluginPartQueryInput, "domain_id">,
  items: PipelinePluginPart[]
) {
  return asItemResult(`Loaded ${items.length} plugin input fields`, {
    id: `${input.plugin_name}@${input.version}:inputs`,
...
type PipelineGetPluginInputsClient = {
  getPluginInputs: (input: PipelinePluginPartQueryInput) => Promise<{
    items: PipelinePluginPart[];
  }>;
};

export function createPipelineGetPluginInputsHandler(client: PipelineGetPluginInputsClient) {
