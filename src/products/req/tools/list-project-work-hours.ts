...
type ReqListProjectUserWorkHoursClient = {
...
export function createReqListProjectUserWorkHoursHandler(client: ReqListProjectUserWorkHoursClient) {
  return async (input: unknown) => {
    const parsed = reqListProjectUserWorkHoursInput.parse(input);
    const response = await client.listProjectUserWorkHours(parsed);
    const result = mapReqProjectWorkHours(
      response.work_hours,
      parsed.page,
      parsed.page_size,
