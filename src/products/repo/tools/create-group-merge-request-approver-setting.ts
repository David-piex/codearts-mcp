...
import { repoCreateGroupMergeRequestApproverSettingInput } from "../schemas.js";
import {
  mapApproverSettings,
  previewApproverSettingsMutation
} from "./merge-request-settings-result.js";

type RepoCreateGroupMergeRequestApproverSettingClient = {
  createGroupMergeRequestApproverSetting: (input: {
    group_id: string;
    id?: string;
    target?: string;
    target_type?: "branch";
...
export function createRepoCreateGroupMergeRequestApproverSettingHandler(client: RepoCreateGroupMergeRequestApproverSettingClient) {
  return async (input: unknown) => {
    const parsed = repoCreateGroupMergeRequestApproverSettingInput.parse(input);

    if (parsed.dry_run) {
      const result = previewApproverSettingsMutation("Dry run: create group merge request approver setting", {
    ...
    const response = await client.createGroupMergeRequestApproverSetting(parsed);
