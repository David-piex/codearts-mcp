...
| Build | 133 | 96 | 37 | Validated | Core Build job, record, log, parameter, domain metadata, permission, code-tag, report, and resource-spec tools are covered by live or smoke validation. Git-code endpoint reads are implemented and unit-tested, with endpoint-specific live samples optional. |
...
- Product tools implemented: `1490`
- Auth/session tools implemented: `2`
- Total MCP tools exposed: `1492`
...
| Check | 96 | `14 Full / 0 Reachable / 0 Unpublished / 5 Code` | Core Check closure remains usable for task/ruleset/metrics/defect workflows; official read routes are live-validated, while dry-run-safe write/trigger tools still need dedicated real-write samples. |
...
| Build | 133 | Expanded metadata read surface with live smoke coverage | The Build surface now includes metadata reads for domain status, permissions, code tags, report repositories/branches, resource specs, and optional Git-code repositories/branches. |
