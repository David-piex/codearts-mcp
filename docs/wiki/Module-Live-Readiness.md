...
| Check | 69 | 64 | 5 | Validated | Core Check task/ruleset/metrics/defect reads are AK/SK validated. The newly added official read routes for plugins, task webhook info, code-health SVG, criterion filters, criterions, and defect task statistics are also live-validated on the codearts-check Beijing 4 endpoint. |
...
| Build | 87 | 78 | 9 | Validated | Core Build job, record, log, parameter, domain metadata, permission, code-tag, report, and resource-spec tools are covered by live or smoke validation. Git-code endpoint reads are implemented and unit-tested, with endpoint-specific live samples optional. |
...
- Product tools implemented: `1044`
- Auth/session tools implemented: `2`
- Total MCP tools exposed: `1046`
...
| Check | 69 | `14 Full / 0 Reachable / 0 Unpublished / 0 Code` | Core Check closure remains usable for task/ruleset/metrics/defect workflows; the six official read routes are now live-validated on the codearts-check endpoint. |
...
| Build | 87 | Expanded metadata read surface with live smoke coverage | The Build surface now includes metadata reads for domain status, permissions, code tags, report repositories/branches, resource specs, and optional Git-code repositories/branches. |
