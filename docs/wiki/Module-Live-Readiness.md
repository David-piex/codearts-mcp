...
| --- | --- | --- | --- | --- | --- |
| Req | 42 | 19 | 23 | Partial | Req now covers Scrum project, module, member, iteration, work-item, comment, association, related-user, status, workflow-config, and flow-transition tools. Core project/work-item paths have live coverage; member, batch-operation, and newly added config-read live depth is still being expanded. |
...
- Product tools implemented: `252`
- Auth/session tools implemented: `2`
- Total MCP tools exposed: `254`
...
| --- | --- | --- | --- |
| Req | 42 | Expanded Scrum surface with configuration read support | The Req MCP surface has grown from the original 8-tool core to project/module/member/iteration/work-item collaboration plus status/workflow configuration reads; core live loops are validated while deeper member, batch, and config-read samples remain pending. |
