...
| Check | 26 | 22 | 4 | Validated | Full tool-level live loop completed. |
...
| Build | 53 | 44 | 9 | Validated | Core Build job, record, log, parameter, domain metadata, permission, code-tag, report, and resource-spec tools are covered by live or smoke validation. Git-code endpoint reads are implemented and unit-tested, with endpoint-specific live samples optional. |
| Artifact | 23 | 21 | 2 | Partial | Five tools are AK/SK Full; the remaining seven are re-confirmed as unpublished in Beijing 4. |
...
- Product tools implemented: `865`
- Auth/session tools implemented: `2`
- Total MCP tools exposed: `867`
...
| Check | 26 | `8 Full` | Tool-level closure is complete. |
...
| Build | 53 | Expanded metadata read surface with live smoke coverage | The Build surface now includes metadata reads for domain status, permissions, code tags, report repositories/branches, resource specs, and optional Git-code repositories/branches. |
| Artifact | 23 | `5 Full / 0 Reachable / 7 Unpublished / 0 Code` | Five tools are fully validated; seven routes are unpublished in Beijing 4. The current tenant now exposes a real published file sample at `/codearts-mcp/1.0.0/codearts-mcp.tgz`. |
