...
| Check | 82 | 14 | Partial | Core Check task/ruleset/metrics/defect reads and the six official read routes are AK/SK validated; newer write/trigger configuration tools are dry-run safe and still need dedicated real-write samples |
| TestPlan | 253 | 41 | Partial | `get_plan / list_runs / get_case / run_cases` are unpublished in Beijing 4 |
...
| Build | 96 | 37 | Validated | Core Build job, record, log, parameter, domain metadata, permission, code-tag, report, and resource-spec tools have live or smoke coverage; optional Git-code endpoint reads need endpoint-specific samples |
