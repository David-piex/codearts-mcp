...
| --- | --- | --- | --- | --- |
| Req | 24 | 23 | Partial | Scrum project, module, member, iteration, work-item, comment, association, status, workflow-config, template, template-config, custom-field, status-rule-flag, and tracker-handler tools are implemented; deeper live coverage is still expanding for member, batch, and new config-read paths |
