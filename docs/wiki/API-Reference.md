...
| TestPlan | 241 | 231 | 10 | `HUAWEICLOUD_TESTPLAN_BASE_URL` |
| Deploy | 74 | 57 | 17 | `HUAWEICLOUD_DEPLOY_BASE_URL` |
| Build | 95 | 86 | 9 | `HUAWEICLOUD_BUILD_BASE_URL` |
| Artifact | 48 | 42 | 6 | `HUAWEICLOUD_ARTIFACT_BASE_URL` |

产品工具合计 `1089` 个。HTTP 共享模式额外提供 `auth_configure_session` 和 `auth_clear_session` 两个会话工具，因此 HTTP MCP 总工具数为 `1091`。
