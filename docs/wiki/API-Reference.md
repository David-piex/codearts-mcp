...
| --- | ---: | ---: | ---: | --- |
| Req | 342 | 208 | 134 | `HUAWEICLOUD_REQ_BASE_URL` |
| Repo | 187 | 141 | 46 | `HUAWEICLOUD_REPO_BASE_URL` |
| Pipeline | 109 | 72 | 37 | `HUAWEICLOUD_PIPELINE_BASE_URL` |
...
产品工具合计 `1236` 个。HTTP 共享模式额外提供 `auth_configure_session` 和 `auth_clear_session` 两个会话工具，因此 HTTP MCP 总工具数为 `1238`。
