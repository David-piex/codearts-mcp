...
| Pipeline | 108 | 72 | 36 | `HUAWEICLOUD_PIPELINE_BASE_URL` |
| Check | 64 | 60 | 4 | `HUAWEICLOUD_CHECK_BASE_URL` |
...
产品工具合计 `1026` 个。HTTP 共享模式额外提供 `auth_configure_session` 和 `auth_clear_session` 两个会话工具，因此 HTTP MCP 总工具数为 `1028`。
