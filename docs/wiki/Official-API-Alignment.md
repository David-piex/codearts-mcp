...
| Repo | 24 | 约 321 | Selective subset | 聚焦仓库、分支、提交、文件、MR、Tag 等常用操作 |
| Pipeline | 16 | 约 225 | Selective subset | 聚焦流水线执行、审核、日志、产物、模板列表 |
...
当前 Repo MCP 已经覆盖了最常用的仓库协作主链, 但和官方 Repo PDF 的总能力面相比仍然只是精选子集。
...
当前 Pipeline MCP 是“执行与排障优先”的能力面, 和官方文档语义一致, 但不是完整产品面。
