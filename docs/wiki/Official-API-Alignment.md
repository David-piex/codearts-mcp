...
- `IssueDetailsV2 /v2/issues/show` 是 Req 工作项详情的官方原始接口；当前 MCP 的 `req_get_work_item_issue_details` 直接读取该 V2 详情，并把 `journals` 映射为 `comments`，同时保留 `assignee` / `assignedToName` 便于查看处理人。
