...
| --- | --- | --- | --- |
| Req | 52 | 已对齐到 Scrum 高频实用层，并开始进入看板与字段缓存读面 | IPD / 需求池 / 看板更深写面 / 更深字段配置 / 附件等仍未进入 MCP；新增工具的 live 仍需继续补 |
...
- 当前对齐到官方 Req API 的 `Scrum 高频实用层`，并开始进入看板项目工作项读面与字段缓存读面，不再只是旧文档里的 8 个核心工具，而是已经覆盖 `project / module / member / iteration / work-item / collaboration / config-read / board-read / cache-read` 九个资源面，共 `52` 个工具
...
- `board-read` 面已经补到 `req_list_board_work_items`、`req_list_board_work_item_status_records`、`req_list_board_work_item_workflow_config`，能直接读取看板项目工作项列表、状态历史和流转配置
- `cache-read` 面已经补到 `req_list_job_cache_boards`、`req_list_cache_data`，能直接读取卡片模式字段缓存和 backlog 类字段缓存数据
...
- 明确还没进入当前对齐范围的方向包括：IPD、需求池、项目空间/看板、更深的字段配置写面与域级管理、工时、附件传输
